document.addEventListener('DOMContentLoaded', function () {
    ...
    if (compareBtn) {
        compareBtn.addEventListener('click', () => {
            const tyreId = compareBtn.dataset.tyreId;

            if (tyreId) {
                // Redirect to the comparison page with this tyre ID
                window.location.href = `price_compare.php?tyre_id=${tyreId}`;
            } else {
                alert('Tyre ID not found for comparison.');
            }
        });
    }
