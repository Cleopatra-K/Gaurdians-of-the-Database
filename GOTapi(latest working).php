...
class ProductAPI {
            ...
            switch ($requestType) {
                ...
                case "addFavourite":
                    ...
                    if (!isset($data["tyre_id"])) {
                        $this->sendErrorResponse("Missing required parameter for adding favourite: tyre_id.", 400);
                        break;
                    }
                ...
                case 'SubmitRating':
                    // Authorize based on the determined $user
                    if ($user['user_id'] === null) { // Ratings require a logged-in user
                        $this->sendErrorResponse("Unauthorized: Login required to submit ratings.", 401);
                        break;
                    }
                    $this->submitRating($data, $user['user_id']); // Pass user_id
                    break;
    ...
    private function addFavourite($user_id, $tyreId) {
        try {
            ...
            $stmt = $this->connection->prepare("INSERT INTO favourites (user_id, tyre_id, added_at) VALUES (?, ?, NOW())");
            if (!$stmt) {
    ...
    private function submitRating($data, $user_id) {
        try {
            $requiredFields = ['tyre_id', 'rating'];
            foreach ($requiredFields as $field) {
                if (!isset($data[$field]) || empty($data[$field])) {
                    throw new Exception("Missing required field for SubmitRating: $field", 400);
                }
            }

            $tyreId = intval($data['tyre_id']);
            $rating = intval($data['rating']);
            $review = $data['review'] ?? null; // Optional review text
            ...
            $stmt = $this->connection->prepare("SELECT tyre_id FROM products WHERE tyre_id = ?");
            if (!$stmt) {
                throw new Exception("SQL prepare failed for submitRating (check tyre): " . $this->connection->error, 500);
            }
            $stmt->bind_param("i", $tyreId);
            $stmt->execute();
            if ($stmt->get_result()->num_rows === 0) {
            ...
            $stmt = $this->connection->prepare("SELECT rating_id FROM ratings WHERE user_id = ? AND tyre_id = ?");
            if (!$stmt) {
                throw new Exception("SQL prepare failed for submitRating (check existing): " . $this->connection->error, 500);
            }
            $stmt->bind_param("ii", $user_id, $tyreId);
            $stmt->execute();
            if ($stmt->get_result()->num_rows > 0) {
                ...
                $stmt->close();
                $stmt = $this->connection->prepare("UPDATE ratings SET rating = ?, review_text = ?, rated_at = NOW() WHERE user_id = ? AND tyre_id = ?");
                if (!$stmt) {
                    throw new Exception("SQL prepare failed for update rating: " . $this->connection->error, 500);
                }
                $stmt->bind_param("isii", $rating, $review, $user_id, $tyreId);
                $message = "Rating updated successfully!";
                $statusCode = 200;
            } else {
                // Otherwise, insert new rating
                $stmt->close();
                $stmt = $this->connection->prepare("INSERT INTO ratings (user_id, tyre_id, rating, review_text, rated_at) VALUES (?, ?, ?, ?, NOW())");
                if (!$stmt) {
                    throw new Exception("SQL prepare failed for insert rating: " . $this->connection->error, 500);
                }
                $stmt->bind_param("iiss", $user_id, $tyreId, $rating, $review);
            ...
            if ($stmt->execute()) {
                ...
                throw new Exception("Failed to submit/update rating: " . $stmt->error, 500);
            }
        } catch (Exception $e) {
            error_log("Error in submitRating: " . $e->getMessage());
            $this->sendErrorResponse($e->getMessage(), $e->getCode() ?: 500);
        }
    }

    private function getProductRating($data) {
        try {
            if (!isset($data['tyre_id'])) {
                throw new Exception("tyre_id is required for GetProductRating", 400);
            }
            $tyreId = intval($data['tyre_id']);
            ...
            ");
            if (!$stmt) {
                throw new Exception("SQL prepare failed for getProductRating: " . $this->connection->error, 500);
            }
            $stmt->bind_param("i", $tyreId);
            $stmt->execute();
            ...
            $averageRating = round($result['average_rating'] ?? 0, 2);
            ...
            error_log("Error in getProductRating: " . $e->getMessage());
            $this->sendErrorResponse($e->getMessage(), $e->getCode() ?: 500);
        }
    }

    private function getAllProductRatings($data) {
        try {
            if (!isset($data['tyre_id'])) {
                throw new Exception("tyre_id is required for GetAllProductRatings", 400);
            }
            $tyreId = intval($data['tyre_id']);
            ...
            ");
            if (!$stmt) {
                throw new Exception("SQL prepare failed for getAllProductRatings: " . $this->connection->error, 500);
            }
            $stmt->bind_param("i", $tyreId);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            error_log("Error in getAllProductRatings: " . $e->getMessage());
            $this->sendErrorResponse($e->getMessage(), $e->getCode() ?: 500);
        }
    }
